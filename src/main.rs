...
async fn main() -> Result<(), NockAppError> {
    ...
    let poke = match &cli.command {
        // Peek Commands
        Commands::PeekBalance { pubkey } => return do_peek(Wallet::peek_balance(&pubkey)?.0, wallet_instance.app).await,
        ...
        Commands::AeroeSpend {
            ...
            fee,
            file_path,
        } => Wallet::aeroe_spend(names.clone(), recipients.clone(), gifts.clone(), *fee, file_path.clone()),
